...
- _3단계 - 사용자 풀에 소셜 로그인 추가_
	...
	이 과정은 어플을 사용하는 사용자들이 로그인할 때, Facebook 이나 Google 이나 Amazon 같은 __소셜 자격 증명 공급자(IdP)__ 를 통해서 로그인할 수 있도록 해주는 과정이다. 

	2단계에서 확인해본 내장된 호스트 웹 UI를 사용하여 Amazon Cognito가 모든 자격 증명 공급자(페이스북, 구글 등)의 모든 인증 사용자(어플을 사용하는 사용자)에 대한 토큰 처리 및 관리를 제공한다.
