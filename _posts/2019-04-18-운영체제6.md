...
- [Critical Section 문제를 해결하기 위해 만족되어야 하는 조건들]
...
- _Peterson's Solution(피터슨의 해결책)_
	...
	그런데 이 때, 만약 flag[1](=P2)이 준비상태이면서 turn이 P2차례라면 P1은 critical section에 들어가지 못하고 while문을 계속 돈다.(while문을 통과 해야 critical section에 들어갈 수 있다)
...
- _Swap 함수 사용_
	...
	즉, key가 true라면 key를 false로 바꾼 후, critical section에 들어가는 것이다.
...
- _Semaphore 사용_
	...
	그러다가 S가 1 이상이 되면 while문을 벗어난다. 동시에 S를 1만큼 감소시키는 함수이다.

	V() 함수는 변수S를 1만큼 증가시키는 함수이다.

	이제 이 두 함수를 이용해서 critical section에 한 프로세스만 들어오도록 조작하는 아래 코드를 이해해보자!

	![운영체제50](https://user-images.githubusercontent.com/31889335/56378658-a95b2180-6248-11e9-816c-00ec5a272f07.PNG)
