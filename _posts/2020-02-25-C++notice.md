...
cin 객체에 대해서 알아보니 cin과 cout은 연결되어 있고, 으로 데이터를 읽을 때 먼저 출력 버퍼를 비워준다는 사실을 알게되었다. 
...
하지만 cin 입력 전에 출력버퍼를 비우는 과정에서 시간이 추가로 소요되는 점이 cin 객체가 느려지는 이유 중 하나이다.

따라서 cin 객체의 속도를 빠르게 하기 위해서는 다음과 같이 cin과 cout이 연결되어 있는 것을 끊어주면 된다.
